...
class convert:
    ...
    def __init__(self, sourcefile, cp_in, cp_out, sheet, sep, ext):
    ...
    def write_from_excel(self):
        print(f'Reading data from "{os.path.basename(self.sourcefile)}"')
        try:
        ...
        for lsheet in excelFile:
            finalfile = os.path.splitext(
                self.sourcefile)[0] + '_' + lsheet + '.dbf'
            df = pd.DataFrame(excelFile[lsheet])
            if len(df) > 0:
                self.save_dbf(df, finalfile)
                sheetCnt += 1
        if sheetCnt > 1:
            self.messages("The file contains more than one sheet...")

    def write_from_csv(self):
        finalfile = os.path.splitext(self.sourcefile)[0] + '.dbf'

        print(
            f'Reading data from "{self.sourcefile}" with encoding {self.cp_in}'
        )
    ...
    def save_dbf(self, df, finalfile):
        ...
        print(
            f'Writing {reccount} records into "{finalfile}" with encoding {self.cp_out}'
        )
        try:
            if os.path.isfile(finalfile):
                os.remove(finalfile)
            dbfTable = dbf.Table(filename=finalfile,
